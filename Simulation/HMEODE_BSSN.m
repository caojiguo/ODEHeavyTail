% Simulation 1
% Random effects were generated from the Birnbaum-Saunders distribution
% shape=0.5 and scale=0.5;
...
%bImat = zeros(2,ncohort);

BS_alpha=0.5; BS_beta=1;
BS_mean=BS_beta*(1+BS_alpha^2/2);
BS_var = (BS_alpha*BS_beta)^2*(1+5*BS_alpha^2/4);
...
for tt=1:Nreps
    ...
    Z  = normrnd(0,1,2,ncohort);
    T  = BS_beta/4*(BS_alpha*Z+sqrt((BS_alpha*Z).^2+4)).^2;
    
    temp1 = 1/sqrt(BS_var)*(T-BS_mean);
    temp2 = Amat*temp1;
    thetaImat(1,:) = truea+temp2(1,:);
    thetaImat(2,:) = trueb+temp2(2,:);
    

      Z  = normrnd(0,1,ncohort,1);
      T  = BS_beta/4*(BS_alpha*Z+sqrt((BS_alpha*Z).^2+4)).^2;
      X0 = 1+1/sqrt(BS_var)*(T-BS_mean);
...
lambdakappaSam_TT = zeros(Final_size,1);
lambdanuSam_TT = zeros(Final_size,1);
...
for iter=1:ITER
  ...
  if mod(iter,10)==0   
    ...
    nuSam_TT(ctr1)         = oldnu;
    lambdakappaSam_TT(ctr1) = oldlambda_kappa;
    lambdanuSam_TT(ctr1)    = oldlambda_nu;
