% Simulation 1
% Random effects were generated from the mixture of Student's t distributions;
% 0.6*t(3)+0.4*t(6);
...
for tt=1:Nreps
    ...
    sigma_t = sqrt(0.6*3/(3-2)+0.4*6/(6-2));
    
    w1 = trnd(3,1,ncohort);
    u1 = rand(1,ncohort);
    w1(u1<0.4) = trnd(6, 1,sum(u1<0.4));
    w2 = trnd(3,1,ncohort);
    u2 = rand(1,ncohort);
    w2(u2<0.4) = trnd(6, 1,sum(u2<0.4));

    bImat(1,:)=w1;
    bImat(2,:)=w2;
    
    thetaImat = truepar*ones(1,ncohort)+Amat/sigma_t*bImat;
    
%     
%     y0I = y0+0.2/sqrt(2)*trnd(4,ncohort,1);
      w3 = trnd(3,1,ncohort);
      u3 = rand(1,ncohort);
      w3(u3<0.4) = trnd(6, 1,sum(u3<0.4));
      X0 = 1+0.2/sigma_t*w3;
