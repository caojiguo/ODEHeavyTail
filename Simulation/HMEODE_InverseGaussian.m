% Simulation 1
% Random effects were generated from the inverse Gaussian distribution
% location=1.0 and scale=1.0;
...
lambda = 1;
pd = makedist('InverseGaussian','mu',1,'lambda',lambda);
sigma_IG = sqrt(1/lambda);  
...
for tt=1:Nreps
    ...
    w1 = random(pd,1,ncohort)-1;
    w2 = random(pd,1,ncohort)-1;
    

    bImat(1,:)=w1;
    bImat(2,:)=w2;
    
    thetaImat = truepar*ones(1,ncohort)+Amat/sigma_IG*bImat;
    
     w3 = random(pd,1,ncohort)-1;
     X0 = 1+0.2/sigma_IG*w3;
